/**
 * XPM Image SEO Admin JavaScript - Complete Version
 */
jQuery(document).ready(function($) {
    ...
    function initializeOptimizer() {
        ...
        function updateStatsDisplay(stats) {
            $('#total-images').text(stats.images_optimized || 0);
            ...
            if (stats.images_optimized > 0) {
                $optimizerStats.show();
                addOptimizerLogEntry(`📊 Previously optimized: ${stats.images_optimized} images, saved ${stats.total_savings_human}`, 'info');
            }
        }
    ...
    function initializeBulkUpdate() {
        ...
        $scanButton.on('click', function(e) {
            ...
            $.ajax({
                ...
                method: 'POST',
                data: {
                    action: 'xmp_get_images_without_alt',
                    nonce: xpmImageSeo.nonce
                },
                success: function(response) {
                    if (response.success) {
        ...
        function addLogEntry(message, type = 'info') {
            ...
            if ($resultsLog.children().length > 100) {
                $resultsLog.children().slice(100).remove();
            }
