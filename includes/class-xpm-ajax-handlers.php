<?php
/**
 * XPM Image SEO AJAX Handlers - COMPLETE FIXED VERSION
 * 
...
class XPM_AJAX_Handlers {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_xmp_restore_from_backup', array($this, 'ajax_restore_from_backup'));
        
        // Debug: Log AJAX handlers registration
        error_log('XPM Image SEO AJAX Handlers Registered Successfully');
        error_log('Registered AJAX actions: xpm_get_images_without_alt, xpm_bulk_update_alt_text, xpm_get_unoptimized_images, xpm_bulk_optimize_image, xpm_get_optimization_stats, xpm_restore_from_backup');
    }
    ...
    public function ajax_bulk_update_alt_text() {
        error_log('AJAX bulk_update_alt_text called with data: ' . print_r($_POST, true));
    ...
    public function ajax_bulk_optimize_image() {
        error_log('AJAX bulk_optimize_image called with data: ' . print_r($_POST, true));
        ...
        try {
            ...
            if ($result['success']) {
                ...
                error_log('Image optimization successful for attachment ' . $attachment_id);
                wp_send_json_success($result);
            } else {
                error_log('Image optimization failed for attachment ' . $attachment_id . ': ' . $result['message']);
                wp_send_json_error($result['message']);
            }
        } catch (Exception $e) {
            error_log('Image optimization exception for attachment ' . $attachment_id . ': ' . $e->getMessage());
    ...
    public function ajax_restore_from_backup() {
        error_log('AJAX restore_from_backup called with data: ' . print_r($_POST, true));
        ...
        try {
            ...
            if ($result['success']) {
                error_log('Image restore successful for attachment ' . $attachment_id);
                wp_send_json_success($result);
            } else {
                error_log('Image restore failed for attachment ' . $attachment_id . ': ' . $result['message']);
                wp_send_json_error($result['message']);
            }
        } catch (Exception $e) {
            error_log('Image restore exception for attachment ' . $attachment_id . ': ' . $e->getMessage());
