<?php
/**
 * XPM Image SEO Media Library Integration - COMPLETE FIXED VERSION
 * 
...
class XPM_Media_Library_Integration {
    ...
    public function __construct() {
        ...
        // FIXED: Load scripts for media library
        add_action('admin_enqueue_scripts', array($this, 'enqueue_media_scripts'));
    }
    
    /**
     * Enqueue scripts for media library - FIXED VERSION
     */
    public function enqueue_media_scripts($hook) {
        // Load on media library pages and attachment edit modal
        if (in_array($hook, array('upload.php', 'post.php', 'post-new.php')) || 
            (isset($_GET['post']) && get_post_type($_GET['post']) === 'attachment')) {
            
            wp_enqueue_script(
                'xpm-media-library', 
                XPM_IMAGE_SEO_PLUGIN_URL . 'assets/js/media-library.js', 
                array('jquery'), 
                XPM_IMAGE_SEO_VERSION, 
                true
            );
            
            wp_localize_script('xpm-media-library', 'xpmImageSeo', array(
                'ajax_url' => admin_url('admin-ajax.php'),
                'nonce' => wp_create_nonce('xpm_image_seo_nonce'),
                'strings' => array(
                    'generating' => __('Generating...', 'xpm-image-seo'),
                    'optimizing' => __('Optimizing...', 'xpm-image-seo'),
                    'restoring' => __('Restoring...', 'xpm-image-seo'),
                    'confirm_restore' => __('Restore original image? This will undo all optimizations.', 'xpm-image-seo'),
                    'confirm_reoptimize' => __('Re-optimize this image? This will overwrite the current optimization.', 'xpm-image-seo'),
                    'success_generated' => __('Alt text generated successfully!', 'xpm-image-seo'),
                    'success_optimized' => __('Image optimized successfully!', 'xpm-image-seo'),
                    'success_restored' => __('Original image restored successfully!', 'xpm-image-seo'),
                    'error_network' => __('Network error. Please try again.', 'xpm-image-seo'),
                    'error_invalid_id' => __('Invalid attachment ID', 'xpm-image-seo')
                )
            ));
        }
        
        // Also load on media modal/grid view
    ...
    /**
     * Add fields to attachment edit screen - FIXED VERSION
     */
    public function add_attachment_fields($fields, $post) {
        ...
        // Quick action buttons with FIXED class names
